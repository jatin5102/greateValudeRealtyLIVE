...
class ProjectBanner extends Model
{
    ...
    public function getDesktopVideoAttribute()
    {
        if(!empty($this->attributes['desktop_video'])){
            if(File::exists(public_path('storage/'.$this->attributes['desktop_video']))){
                return asset('storage/'.$this->attributes['desktop_video']);
            } 
        }
    ...
    public function getMobileVideoAttribute()
    {
        if(!empty($this->attributes['mobile_video'])){
            if(File::exists(public_path('storage/'.$this->attributes['mobile_video']))){
                return asset('storage/'.$this->attributes['mobile_video']);
            } 
        }
    }
