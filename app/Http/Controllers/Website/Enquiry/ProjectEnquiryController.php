    ...
    public function store (Request $request)
    {
        ...
        if($validator->fails()){
            ...
            try{
                ...
                if(!$getproject && $request->project_id == 'iconic'){
                    $projectName = 'ekanam';
                    $record->project_id = '972';
                } else {
                ...
                // $client = new Client();
                // $response = $client->request('GET', 'https://api2.gtftech.com/AjaxHelper/AgentInstantQuerySetter.aspx?qAgentID='.env('AGENT_ID').'&qSenderName='.$request->name.'&qMobileNo='.$request->phone.'&qEmailID='.$request->email.'&qQueryMessage='.$request->message.'&qProjectName='.$projectName.'');
