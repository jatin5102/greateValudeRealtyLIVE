...
use App\Models\Website\Project\ProjectMasterPlan;
...
class ProjectController extends Controller
{
    ...
    ->with(['startingPrice' => function($query) {
        ...
        $result =$project->orderBy('seq')->paginate($perPage, ['*'], 'page', $page);
    ...
    public function FloorPlan($project_id){
        $floorplans = ProjectFloorPlan::with('subTypology')->where('project_id',$project_id)->orderBy('type')->get()->groupBy('type');
        return response()->json([
            'status' => true,
            'statusCode' => 200,
            'message' => "Success",
    ...
    public function MasterPlan($project_id){
        $floorplans =ProjectMasterPlan::where('project_id',$project_id)->get();
        return response()->json([
            'status' => true,
            'statusCode' => 200,
            'message' => "Success",
    ...
    public function LocationAdvantage($project_id){
        ...
        $floorplans = ProjectLocationAdvantage::where('project_id', $project_id)->orderBy('seq', 'ASC')->get()->groupBy('type');
    ...
    public function Specifications ($project_id){
        ...
        $specifications = ProjectSpecification::with([
            'ProjectSpecificationList' => function ($query) use ($project_id) {
    ...
    public function ProjectImages(Request $request) {
        ...
        try {
            ...
            $projects->each(function ($project) use ($perPage, $request) {
                $pageParam = 'page_' . $project->id;  // Unique page parameter per project
                $page = $request->input($pageParam, 1);  // Read page for this project only
                
                $project->projects_galleries = $project->projectGallery()
                    ->paginate($perPage, ['*'], $pageParam, $page)
                    ->withQueryString();  // Keep query parameters
            });
    ...
    public function getCommercialPlatter (Request $request) {
        ...
        $category = \App\Models\Website\Categories::with([
            'typologies.galleries' => function ($query) {
                $query->withoutTrashed(); // Ensure no soft-deleted galleries are fetched
            }
        ])->find(2);
        $typologiesWithGroupedGalleries = $category->typologies->sortBy('seq')->values()->map(function ($typology) {
            return [
                'typologies' => [
                    'id' => $typology->id,
                    'name' => $typology->typology,
                    'description' => $typology->description,
                    'existing_area' => $typology->existing_area,
                    'expansion_area' => $typology->expansion_area
                ],
    ...
    public function getCategory(){
        ...
        $categories = \App\Models\Website\Categories::get();
        $projects = Projects::where('is_feature', 1)->limit(1)->get();

        $data = [
            'categories' => $categories,
            'projects'   => $projects,
        ];
        // $mergedData = array_merge($categories, $projects);
